...
import numpy as np
from contarg.utils import cluster, build_bidsname, parse_bidsname, update_bidspath, get_rel_path, make_rel_symlink
import pytest
import tempfile
import shutil
...
def test_parse_and_build_bidsname():
    ...
    assert build_bidsname(parse_bidsname(test_name)) == 'func/sub-02_ses-1_task-rest_acq-1.2.3.4_run-01_echo-2_space-T1w_desc-preproc_bold.nii.gz'
...
def test_update_bidspath():
    ...
    result = update_bidspath(orig_path, new_bids_root, updates, exists=False, exclude=exclude, order=order)
    ...
    result = update_bidspath(orig_path, new_bids_root, updates, exists=False, exclude=exclude)
    ...
    new_bids_root = '/data/bids'
    updates = {'sub': '02', 'ses': '02', 'acq': 'mag', 'type': 'func', 'suffix': 'bold', 'task':'rest'}
    exclude = []
    expected_result = '/data/bids/sub-02/ses-02/func/sub-02_ses-02_task-rest_acq-mag_bold.nii.gz'
    ...
    with pytest.raises(FileNotFoundError):
        result = update_bidspath(orig_path, new_bids_root, updates, exists=True, exclude=exclude)
